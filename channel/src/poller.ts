...
export function startPoller(options: PollerOptions): { stop: () => void } {
  ...
  let servers: ServerSubscription[] = [];
  ...
  async function pollServer(
    ...
    if (resp.status === 403) return; // not subscribed — skip
  ...
  function backoffMs(): number {
    const delay = Math.min(
      BACKOFF_BASE_MS * Math.pow(2, consecutiveFailures),
      MAX_BACKOFF_MS
    );
  ...
  async function pollLoop(): Promise<void> {
    ...
    if (!token) {
      console.error(
        "[mctx-channel] unable to authenticate. channel inactive."
      );
      return;
    }
    ...
    if (servers.length === 0) {
      console.error(
        "[mctx-channel] no channel-enabled server subscriptions found. polling inactive."
      );
    } else {
      console.error(
        `[mctx-channel] polling ${servers.length} server(s): ${servers.map((s) => s.slug).join(", ")}`
      );
    }
    ...
    while (running) {
      try {
        ...
        if (!token) {
          console.error(
            "[mctx-channel] token expired, re-authenticating..."
          );
        ...
        for (const server of servers) {
          ...
          try {
            ...
            console.error(
              `[mctx-channel] error polling ${server.slug}:`,
              err
            );
          }
        }
        ...
        console.error(
          `[mctx-channel] poll error (attempt ${consecutiveFailures}, retry in ${Math.round(delay / 1000)}s):`,
          err
        );
        await sleep(delay);
      }
